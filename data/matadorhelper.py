...
token = "387870057:AAEzOZQPADmtv15aS07TaLLj83NniobzLqY"
	...
	if call.data:
		...
		if str(call.from_user.id) in mods or str(call.from_user.id) in owner:
			...
			if call.data == "about"+str(target):
				if not lang:
					markup = types.InlineKeyboardMarkup()
					n = types.InlineKeyboardButton(text="NiKooTin", url="https://telegram.me/nikootins")
					markup.add(n)
					...
					markup = types.InlineKeyboardMarkup()
					a = types.InlineKeyboardButton(text="NiKooTin", url="https://telegram.me/nikootins")
                                        markup.add(a)
